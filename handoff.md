...
同日依潘醫師回饋迭代（皆已上線）：好料擴充為 **10 種**——加 👪 家人篩檢與手繪牙刷杯
（「不共用牙刷」畫成杯子插一支側面圖示牙刷，單支牙刷試過兩版都不像）、排裝藥、14 天日曆；
**抽血試管 NG 品做過又撤掉**（要先懂「複檢不能抽血」才看得懂，太迂迴），改成 **🛡️ 護胃盾**
好料（+20、必跳「除菌護胃！降低胃癌風險」，正面傳達預防胃癌）；`body{user-select:none}`
修拖曳反白。遊戲已經潘醫師實玩，圖案回饋均已處理。

**2026-08-28：衛教單張螢幕版面改版＋遊戲尺寸全站統一。**
潘醫師指示遊戲版面要放大、三站尺寸統一、文字內容點了才出現，指定以
`osteoporosis-clinic/patient-education.html` 的版型為準。完成：

- `src/leaflet.template.html` 螢幕版面重構：hero 大標頭卡片＋分頁膠囊
  （🎮 先來闖關／1–5 章／帶走三句話），預設停在闖關遊戲，文字內容點分頁才出現；
  通關按鈕從錨點連結改為切到第 1 章分頁
- 遊戲尺寸改用**全站統一規格**（標準出自骨鬆版：標題 27／題目 21／選項 18／星星 38；
  規格全文存在 auto-memory `quiz-uniform-layout`，healthy-weight-public 同日同步）
- **列印輸出完全不變**：CSS 改成雙模式——base 是網頁字級，`@media print` 還原原本
  緊湊值並展開全部分頁，仍是 A4∩Letter 單張、遊戲不進紙本
- 瀏覽器實測通過（分頁切換、答題、星星、無 console 錯誤），已推上線（`21f1c29`）
...
- 時間：2026-08-28（衛教單張版面改版＋遊戲尺寸全站統一）
- 更新者：Claude Code（Fable 5）@ X108521（醫院）
- Git push：✅ 已推（`21f1c29` 改版＋本收工 commit）
- L3 Obsidian：⚠️ 本次在醫院電腦收工，此電腦無 Obsidian vault，L3 未更新
- ⚠️ 改衛教單張樣式要顧**兩種模式**：base CSS 是螢幕網頁版，`@media print` 是紙本版，動字級/間距記得兩邊分開看
- ⚠️ OneDrive `文件\HP-clinic_成品\` 發送用副本在家用電腦——下次在那台開工記得同步最新版衛教單張
- 前一筆：2026-08-27 @ X108521 · ✅ 已推（`378f015`）
