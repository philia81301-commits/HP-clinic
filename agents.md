...
- [ ] **階段一：資料層＋查證**（八成完成）
  ...
  - [x] `data/` 五份：01 流病傳染、02 檢驗判讀與停藥、03 處方與抗藥性、05 複檢追蹤、06 家庭篩檢
  - [ ] **`data/04-健保給付與申報.md`** —— 卡在「治療給付放寬但檢驗給付未放寬」的落差待確認（見 handoff）
  - [ ] 13 條 🔲 待溯源（各檔檔尾），依「補不到就降級」或「補不到就不寫」兩種方式處理
...
│   ├─ 05-複檢與追蹤.md
│   └─ 06-家庭篩檢與同時治療.md
├─ src/               規則引擎與產生腳本（rules.json、build.js…）
