...
- [ ] **階段二：診間決策工具＋病人衛教單張** —— 共用同一份 `src/rules.json`
  ...
  - [x] 2026-08-30 「除菌接接樂」動作小遊戲——獨立頁 `除菌接接樂.html`（repo 根目錄），
        入口在衛教單張五連闖下方連結卡（no-print）；知識卡十則取自衛教單張審定內容；
        遊戲呈現形式待潘醫師過目
  - [x] 2026-08-28 衛教單張螢幕版面改版——hero 大標頭卡片＋分頁膠囊（版型比照 osteoporosis-clinic），
        預設停在「先來闖關」、文字內容點分頁才出現；遊戲尺寸改用全站統一規格
        （標題27/題目21/選項18/星38，三站同步）；列印輸出維持原 A4∩Letter 單張不變
